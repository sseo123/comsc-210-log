Write the following two programs in C++. Upload your cpp and h files where applicable labeled wk14_program1.cpp, wk14_program2.cpp, and wk14_pgm2BinaryTree.h (should contain entire template class with declaration and implementation) respectively.

*Include meaningful comments throughout your code explaining the purpose of variables, logic, and major steps.


PROGRAM 1:
Queue Converter

Write a program that stores a series of numbers in a binary tree. Then have the program insert the values into a queue in ascending order. Dequeue the values and display them on the screen to confirm that they were stored in the proper order.

Example Run 1

Inserting values into the binary tree:
50 30 70 20 40 60 80 

Dequeuing values from queue (should be in ascending order):
20 30 40 50 60 70 80




PROGRAM 2:
Binary Tree Template

Write your own version of a class template that will create a binary tree that can hold values of any data type. Demonstrate the class with a driver program.

Example Run 1 (Integer Tree with Multiple Traversals)

Inserting integers into the tree:
50 30 70 20 40 60 80 

In-order traversal (ascending): 
20 30 40 50 60 70 80

Pre-order traversal: 
50 30 20 40 70 60 80

Post-order traversal: 
20 40 30 60 80 70 50

Example Run 2 (String Tree with Multiple Traversals)

Inserting strings into the tree:
pear apple orange banana 

In-order traversal (alphabetical): 
apple banana orange pear

Pre-order traversal: 
pear apple banana orange

Post-order traversal: 
banana apple orange pear

